...
Other options for compiling Concurrent Wrapper library:
- `make [all | examples]` - compiles examples
- `make performance` - compiles performance measurement tests
...
I developed this library as a part of my other ['Command Line Interface for C++ Applications'](http://github.com/gimesketvirtadieni/cli) project. It is used as the main ‘machinery’ for asynchronous telnet server implementation. Currently this library already provides more capabilities than I need so I do not plan to extend its functionality but rather maintain it by resolving issues once discovered.
